...
transcribeRouter.post("/", rateLimit("authStrict"), upstashTranscribeLimit, async (c) => {
  ...
  try {
    ...
    if (!whisperRes.ok) {
      ...
      return c.json({ error: { message: "Audio transcription failed. Please try again.", code: "WHISPER_ERROR" } }, 500);
    }
    ...
    if (!chatRes.ok) {
      ...
      return c.json({ error: { message: "Event extraction failed. Please try again.", code: "GPT_ERROR" } }, 500);
    }
    ...
    let postProfile = await prisma.userProfile.findUniqueOrThrow({ where: { userId: user.id } });
    postProfile = await ensureProBadgeProgressStarted(postProfile);
    await prisma.userProfile.update({
      where: { id: postProfile.id },
      data: { totalVoiceSessions: { increment: 1 } },
    });
    postProfile = await prisma.userProfile.findUniqueOrThrow({ where: { userId: user.id } });
    ...
    await maybeAwardProBadgesAfterTranscribe(postProfile, events.length, durationSecs, now);
